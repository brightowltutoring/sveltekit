...
<button class="${
    "fixed w-full h-full grid place-items-center z-50 md:py-4 py-1 md:px-[7%] " +
...
<svg aria-label="${"Moon"}" id="${"darkIcon"}"${add_attribute(
      "height",
      size,
      0
    )}${add_attribute(
      ...
      size,
      0
    )} class="${"mx-3"}"><path d="${"M12 3a9 9 0 1 0 9 9c0-.46-.04-.92-.1-1.36a5.389 5.389 0 0 1-4.4 2.26 5.403 5.403 0 0 1-3.14-9.8c-.44-.06-.9-.1-1.36-.1Z"}"></path></svg>`;
  }
);
...
const Navitem = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_showHomeworkModal = subscribe(
    showHomeworkModal,
    (value) => value
  );
  ...
  if (
    $$props.navIconClicked === void 0 &&
    $$bindings.navIconClicked &&
    navIconClicked !== void 0
  )
  ...
  do {
    ...
    $$rendered = `<a data-sveltekit-preload-data${add_attribute(
      "href",
      href,
      0
    )} class="${
      "block font-Nunito font-thin " +
      escape(
...
const Navbar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_showHomeworkModal = subscribe(
    showHomeworkModal,
    (value) => ($showHomeworkModal = value)
  );
  $$unsubscribe_showLoginModal = subscribe(
    showLoginModal,
    (value) => ($showLoginModal = value)
  );
  ...
  do {
    ...
    $$rendered = `
<logo-and-navbar class="${
      "opacity-0 " +
      escape(fadeInToFullOpacity, true) +
      " flex items-center justify-center md:justify-between w-full"
    }"><button class="${"md:translate-y-[0.1rem] md:translate-x-3 hidden md:block text-xl font-Poppins md:text-[min(5.5vw,40px)] active:text-red-600 hover:scale-110 transition-transform selection:bg-transparent"}">THINKSOLVE
    </button>

  
  <ul class="${
    "text-xl grid grid-flow-col place-items-center w-full gap-1 p-2 rounded-md md:rounded-xl md:ml-24 md:p-1 md:w-auto " +
    escape(bgGradientColor, true) +
    " hideScrollBar overflow-x-scroll overflow-y-hidden"
  }">
    
    <li${add_attribute("class", hideIfNotIOS, 0)}><button class="${
      "font-Nunito font-thin text-2xl md:text-xl hover:rounded py-1 px-2 duration-300 hover:shadow-lg " +
      escape($elementColor, true) +
      " hover:bg-indigo-400 hover:text-white active:animate-pulse duration-200 border-b-1 rounded"
    }">App
      </button></li>
    ...
    ${each(Object.keys($routes).slice(0, 5), (KEY) => {
      return `<li${add_attribute(
        "class",
        `${KEY == "home" && hideIfRunningStandalone}`,
        0
      )}${add_attribute(
        "style",
        KEY == "login" &&
        ...
        $$result,
        {
          ...
          navIconClicked:
            ($routes[KEY].href == "/homework" && $showHomeworkModal) ||
    ...
    <li class="${
      "py-2 translate-y-1 scale-125 md:scale-100 " +
      escape(hideIfRunningStandalone, true)
    }">${validate_component(LightDarkMode, "LightDarkMode").$$render(
      $$result,
      {},
      {},
      {}
    )}</li></ul></logo-and-navbar>`;
  } while (!$$settled);
  $$unsubscribe_isDarkMode();
...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_routes = subscribe(routes, (value) => ($routes = value));
  $$unsubscribe_page = subscribe(page, (value) => ($page = value));
  $$unsubscribe_navAppClicked = subscribe(
    navAppClicked,
    (value) => ($navAppClicked = value)
  );
  $$unsubscribe_showLoginModal = subscribe(
    showLoginModal,
    (value) => ($showLoginModal = value)
  ...
  do {
    ...
    $$rendered = `${
      (($$result.head += `<!-- HEAD_svelte-1v8kdsi_START --><link rel="${"manifest"}" href="${"/manifest.json"}">${each(
        Object.keys($routes),
        (key) => {
          return `${
            $page.route.id == "/" && key == "home"
              ? `${
