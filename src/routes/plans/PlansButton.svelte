<script lang="ts">
	...
	async function handlePlansModal(e: MouseEvent, BUTTON_URL: string) {
		...
		setTimeout(() => {
			changeOpacityTo100 = 'opacity-100 transition-opacity duration-200 ease-in';
		}, 25);
	}
</script>
...
<button
	...
	class="m-1 rounded-md p-4 text-xl text-white duration-200 hover:scale-105 hover:rounded-lg hover:shadow-md group-hover:bg-opacity-80 {buttonColor[
		card
